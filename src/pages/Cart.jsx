...
function Cart() {
  ...
  useEffect(() => {
    setTotalAmount(cart.reduce((acc, curr) => acc + curr.price, 0)); //0 zero is initital value od
  }, [cart])
          ...
          (
            <div className="flex gap-16 max-w-6xl p-6 mx-auto flex-wrap lg:flex-nowrap ">
              <div className="lg:w-[70%]">
                {
                  cart.map((item, index) => {
                    return <CartItems key={item.id} item={item} itemIndex={index}  ></CartItems>
                  })
                }
              </div>

              <div className="md:w-[30%] w-full flex flex-col gap-8 justify-between overflow-y-hidden">
                <div className="mt-20">
                <p className="text-xl text-[#166534] uppercase font-[600]">Your Cart</p>
                <p className="text-5xl font-[600] text-[#15803d] uppercase mb-4 overflow-y-hidden">Summary</p>
